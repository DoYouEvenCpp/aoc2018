...
constexpr std::array<const char*, 1129> d = { "[1518-02-08 00:00] Guard #1439 begins shift",
...
const auto parseEntry = [](std::string s)
{
	helper h;
	const auto start_pos = s.find('[') + 1;
	const auto end_pos = s.find(']');
	const auto date = s.substr(start_pos, end_pos);
	sscanf_s(date.c_str(), "%d-%d-%d %d:%d", &h.year, &h.month, &h.day, &h.hour, &h.minute);
	return h;
};
...
int main()
{
	std::map<int, std::vector<sleep_time>> shifts;
	int current_id = 0;
	helper current_info{};
	for (auto const& e : d)
	{
		current_info = parseEntry(e);
		std::string s{ e };
		if (s.find("begins") != std::string::npos)
		{
			const auto id_start_pos = s.find('#') + 1;
			const auto id_end_pos = s.find(' ', id_start_pos);
			current_id = std::stoi(s.substr(id_start_pos, id_end_pos));
		}
		else if (s.find("falls") != std::string::npos)
		{
			if (!shifts.count(current_id))
			{
				shifts[current_id];
			}
			shifts[current_id].push_back({});
			shifts[current_id].back().from = current_info;
		}
		else if (s.find("wakes") != std::string::npos)
		{
			shifts[current_id].back().to = current_info;
		}
	}
	...
	for (auto p : shifts)
	{
		...
		for (auto& e : p.second)
		{
			temp_sum += e.to.minute - e.from.minute;
		}
		if (temp_sum > sum)
		{
			sum = temp_sum;
	...
	std::array<int, 60> matrix;
	std::fill(matrix.begin(), matrix.end(), 0);

	for (auto e : shifts[current_id])
	{
		for (auto i = e.from.minute; i < e.to.minute; ++i)
		{
			matrix[i] += 1;
		}
	}

	auto max_minute = std::distance(matrix.begin(), std::max_element(matrix.begin(), matrix.end()));
	std::cout << current_id << " - " << sum << " answer: " << current_id * max_minute <<  std::endl;

	//second part
	std::map<int, std::array<int, 60>> puzzle;
	for (auto& p : shifts)
	{
		puzzle[p.first];
		for (auto e : p.second)
	...
	for (auto& p : puzzle)
	{
		auto max = *std::max_element(p.second.begin(), p.second.end());
		if (max > current_max)
		{
			current_max = max;
			auto max_minute = std::distance(p.second.begin(), std::max_element(p.second.begin(), p.second.end()));
			minute = max_minute;
			current_id = p.first;
		}
	}
	std::cout << "second puzzle: " << current_id << " - " << minute << "- " << current_max <<  " - " << minute * current_id << std::endl;
	return 0;
}